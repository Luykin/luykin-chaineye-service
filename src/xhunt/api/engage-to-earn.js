  ...
  async (req, res) => {
    try {
      ...
      if (!user) {
        return res
          .status(401)
          .json({ error: "User not found or unauthorized" }); // 用户未登录或不存在
      }
      ...
      if (activity.requiresEvmBound) {
        ...
        if (evms.length === 0) {
          return res
            .status(400)
            .json({ error: "EVM address binding required" }); // 该活动要求先绑定 EVM 地址
        }
      }
        ...
        try {
          ...
          if (!data || typeof data.followers_count !== "number") {
            return res
              .status(502)
              .json({
                error: "External validation failed: incomplete response",
              }); // 外部数据校验失败：返回数据不完整
          }
          ...
          if (data.followers_count < (activity.minFollowers || 0)) {
            return res
              .status(400)
              .json({
                error: `Followers requirement not met: at least ${activity.minFollowers}`,
              }); // 不满足条件：粉丝数量需不少于
          }
        } catch (apiErr) {
          console.error(
            "Signup profile check error:",
            apiErr?.message || apiErr
          );
          return res
            .status(502)
            .json({ error: "External validation request failed" }); // 外部数据校验请求失败
        }
      }
      ...
      if (existing) {
        return res
          .status(409)
          .json({ error: "Already signed up for this activity" }); // 您已报名该活动
      }
