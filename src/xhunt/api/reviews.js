...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const summaryCacheKey = `reviews:summary:${accountId}:onlyKOL:${
        onlyKOL ? 1 : 0
      }`;
      ...
      if (
        cachedSummary &&
        Number.isFinite(cachedSummary.averageRating) &&
        Number.isFinite(cachedSummary.totalReviews)
      ) {
      ...
      if (
        typeof cachedRealTotal === "number" &&
        !Number.isNaN(cachedRealTotal)
      ) {
        realTotalReviews = cachedRealTotal;
      } else {
        realTotalReviews = await XReviewForAccount.count({
          where: { xAccountId: accountId },
        });
        try {
          if (req.redisClient?.setEx) {
  ...
  async (req, res) => {
    try {
      ...
      const { rows: comments, count: totalComments } =
        await XReviewForAccount.findAndCountAll({
          where: whereClause,
...
const blacklist = ["Btc1x99", "1596002796985163776", "Metta8253340353"].map(
  (_) => String(_).toLocaleLowerCase()
);
router.post(
  "/",
  [
...
router.post(
  "/delete",
  [authenticateToken, body("handle").trim().notEmpty(), validateRequest],
  async (req, res) => {
    try {
