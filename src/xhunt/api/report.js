      ...
      if (Array.isArray(reportData.records) && reportData.records.length > 0) {
        ...
        if (ipInfo?.ip || proxyDetection.isProxy) {
          ...
          if (proxyDetection.isProxy) {
            ipParts.push(
              `Proxy: ${proxyDetection.likelihood} (Score: ${proxyDetection.score})`
            );
            if (proxyDetection.indicators.length > 0) {
              ipParts.push(
                `Indicators: ${proxyDetection.indicators
                  .slice(0, 3)
                  .join(", ")}`
              );
            }
        ...
        // 合并所有高延迟请求信息
        const delayMessages = recordsToProcess
          .map((record, index) => {
            const parts = [];
            ...
            if (record.success !== undefined)
              parts.push(`Success: ${record.success}`);
            if (record.statusCode) parts.push(`Status: ${record.statusCode}`);
            if (record.errorMessage) {
              const shortError =
                record.errorMessage.length > 70
                  ? record.errorMessage.substring(0, 70) + "..."
                  : record.errorMessage;
            ...
            if (record.currentUrl)
              parts.push(`CurrentPage: ${record.currentUrl}`);

            // 网络信息
            if (record.networkBefore) {
              const netBefore = record.networkBefore;
              parts.push(
                `NetworkBefore: ${netBefore.effectiveType || "unknown"} (${
                  netBefore.downlink || "unknown"
                }Mbps)`
              );
            }
            if (record.networkAfter) {
              ...
              parts.push(
                `NetworkAfter: ${netAfter.effectiveType || "unknown"} (${
                  netAfter.downlink || "unknown"
                }Mbps)`
              );
            }
            ...
            if (record.deviceInfo) {
              ...
              parts.push(
                `Device: ${device.platform || "unknown"} ${
                  device.userAgent ? device.userAgent.slice(0, 50) : ""
                }...`
              );
            }
        ...
        // 计算平均延迟用于判断严重程度
        const avgDuration =
          recordsToProcess.reduce(
            (sum, r) => sum + (Number(r.duration) || 0),
            0
          ) / recordsToProcess.length;

        // 发送高延迟事件
        req.dataDog.event("Frontend High Delay Report", finalMessage, {
        ...
        if (req.dataDog) {
          req.dataDog.increment("high_delay_report.success", 1, [
            ...
            `proxy:${
              proxyDetection.isProxy
                ? proxyDetection.likelihood.toLowerCase()
                : "no"
            }`,
          ]);
        }
