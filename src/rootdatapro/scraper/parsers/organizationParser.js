...
/**
 * 解析组织页面的 HTML 内容以提取数据。
 * @param {string} htmlContent 页面的 HTML 内容。
 * @param {string} url 正在爬取的完整 URL。
 * @returns {object|null} 包含提取数据的对象，如果解析失败则返回 null。
 */
function parseOrganizationPage(htmlContent, url) {
  try {
    const { dom, nuxtData } = getNuxtData(htmlContent);
    if (!nuxtData) {
    ...
    const infoItems = dom.window.document.querySelectorAll(".info_item");
    for (const item of infoItems) {
      const labelElement = item.querySelector(".label");
      if (labelElement && labelElement.textContent.trim() === "Founded:") {
        const infoTextElement = item.querySelector(".info_text");
        if (infoTextElement) {
          establishment_date = infoTextElement.textContent.trim();
          break; // 找到后即可退出循环
        }
      }
    }
