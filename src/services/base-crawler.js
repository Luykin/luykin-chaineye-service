...
class BaseCrawler {
  ...
  static #getDevTgBotInstance() {
    if (!BaseCrawler.#devTgBotInstance) {
      ...
      BaseCrawler.#devTgBotInstance = new TelegramBot(
        BaseCrawler.#devTgToken || _devTgToken
      );
    }
  ...
  static #getProTgBotInstance() {
    if (!BaseCrawler.#proTgBotInstance) {
      ...
      BaseCrawler.#proTgBotInstance = new TelegramBot(
        BaseCrawler.#proTgToken || _proTgToken
      );
    }
  ...
  async #getUserDataDir() {
    ...
    if (
      !this._userDataDir ||
      nowTime - this._userDataDirUpdateTime >
        twentyMinutesInMilliseconds - 60 * 1000
    ) {
      const childClassName = this.constructor.name || "UnknownClass";
      this._userDataDir = path.join(
        "/tmp",
        `puppeteer-profile-${childClassName}-${nowTime}`
      );
  ...
  async #initBrowserWithProxy(proxy) {
    ...
    return await puppeteer.launch({
      headless: "new",
      args: [`--proxy-server=${proxy.ip}:${proxy.port}`, ...puppeteerArgs],
      userDataDir,
    });
  }
  ...
  static async sendMessageToGroupAllEnv(message) {
    return Promise.all([
      BaseCrawler.#sendMessageToGroup("pro", message),
      BaseCrawler.#sendMessageToGroup("dev", message),
    ]);
  }
...
module.exports.ip1 = ip1;
module.exports.ip2 = ip2;
module.exports.ip3 = ip3;
module.exports.ip4 = ip4;
