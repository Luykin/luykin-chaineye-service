...
function bindDownloadEvents() {
    ...
    // 特定用户下载事件
    const downloadSpecificReviewersBtn = document.getElementById('downloadSpecificReviewersBtn');
    const downloadSpecificReceiversBtn = document.getElementById('downloadSpecificReceiversBtn');
    
    if (downloadSpecificReviewersBtn) {
        downloadSpecificReviewersBtn.addEventListener('click', downloadSpecificReviewersData);
    }
    
    if (downloadSpecificReceiversBtn) {
        downloadSpecificReceiversBtn.addEventListener('click', downloadSpecificReceiversData);
    }
}
...
function generateReviewersCSV(data) {
    ...
    const headers = ['排名', '用户名', '显示名称', '分类', 'KOL排名', '使用次数', '是否KOL'];
...
function generateSpecificReviewersCSV(data) {
    ...
    const headers = ['排名', '用户名', '显示名称', '分类', 'KOL排名', '评论次数', '是否KOL'];
    const rows = data.map((item, index) => [
        index + 1,
        item.username || '未知',
        item.displayName || item.username || '匿名用户',
