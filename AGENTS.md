...
├── api/                          # API路由
│   ├── apikey.js                # API Key管理
│   ├── open.js                  # 对外公开API（需API Key）
│   └── rootdatapro.js           # 内部爬虫管理API
├── middleware/
│   └── proApiKey.js             # API Key认证中间件
├── models/                       # 数据模型（16个表）
│   ├── index.js                 # 模型初始化与关联定义
│   ├── Project.js               # 项目表
...
#### 4.4.2 对外公开 API（`/api/rootdatapro/open`，需 pro-api-key）

| 方法 | 路径 | 说明 | 积分消耗 |
|------|------|------|----------|
| GET | `/ecosystem_map` | 获取生态系统列表 | 50 |
| GET | `/tag_map` | 获取标签列表 | 50 |
| POST | `/projects_by_ecosystems` | 按生态系统查询项目 | 20 |
| POST | `/projects_by_tags` | 按标签查询项目 | 20 |
| POST | `/query_investments` | 查询投资记录 | 30 |
| POST | `/batch_get_details` | 批量获取实体详情 | 2×数量 |
| GET | `/search` | 搜索项目/机构/人物 | 5 |
| GET | `/find_by_x` | 通过X用户名查找 | 2 |
| GET | `/search_investment_by_x` | 通过X用户名查投融资信息 | 10 |

#### 4.4.3 API Key 认证机制

```javascript
// 请求头
pro-api-key: <your_api_key>

// 特殊测试Key（无限额度）
rk_666666888888666666LUYKIN
