...
- **禁止**在生产服务器/生产环境直接执行任何会修改状态的命令，包括但不限于：数据库迁移、SQL 写入、发布/回滚、重启/停止服务、修改配置、删除文件、安装依赖等。
- 连接生产服务器时仅允许查看日志、查看进程/状态、查看只读配置或执行只读诊断命令。
- 如发现生产环境需要迁移、重启、修复数据或其他变更，只能向用户汇报原因、风险和建议命令，由用户自行确认并执行。
...
- 涉及代码理解或定位时，若仓库已启用 CodeGraph，应优先使用 CodeGraph，再进行必要的文件读取。
- 不确定业务意图、影响范围较大或可能破坏数据时，先说明疑点并请求确认，不自行假设执行。
- 区分“已执行验证”和“建议验证”，不要把未运行的命令描述为已验证结果。
- 只运行与本次改动相关的轻量检查；不得自动启动服务或构建，除非用户明确要求。
- 不硬编码、泄露或打印真实密钥、token、密码等敏感信息。
