  ...
  temp.querySelectorAll("*").forEach((el) => {
    ...
    const newStyles = style
      ...
      .filter((rule) => {
        ...
        return !(match[1].trim().toLowerCase() === "color" && isBlack(match[2]));
      });
...
function parseItems(content: string) {
  ...
  return parsed.map((item) => {
    const value = item && typeof item === "object" ? (item as Partial<NacosMessageItem>) : {};
    return {
...
function buildPatchHtml(oldItems: NacosMessageItem[], nextItems: NacosMessageItem[]) {
...
export function NacosMessagesPage() {
  ...
  const sortedItems = useMemo(
    () => [...items].map((item, index) => ({ item, index })).sort((a, b) => Number(b.item.created || 0) - Number(a.item.created || 0)),
    [items]
  );
  ...
  function syncContentFromEditor() {
    ...
    if (!editorEnabledRef.current || index < 0 || suppressEditorChangeRef.current) return;
  ...
  async function loadConfig(nextLang = lang, options: { silent?: boolean } = {}) {
    ...
    setLoading(true);
    try {
      const resp = await fetchNacosConfig({ dataId: DATA_IDS[nextLang], group: "DEFAULT_GROUP" });
      ...
      showToast(error instanceof Error ? error.message : "加载配置失败", "error");
    } finally {
      setLoading(false);
    }
  }
  ...
  function newItem() {
    const item: NacosMessageItem = { created: Date.now(), title: "", type: "all", content: DEFAULT_FOOTER_HTML };
  ...
  function openHtmlModal() {
    ...
    window.setTimeout(() => {
      const textarea = document.getElementById("nacos-html-source") as HTMLTextAreaElement | null;
      textarea?.focus();
      textarea?.select();
    }, 100);
  }
  ...
  function showPublishPreview() {
    ...
    setDiffHint(oldJson === newJson ? "（无变更）" : `（${delta > 0 ? "+" : ""}${delta} 条）`);
  ...
  async function doPublish() {
    ...
    try {
      ...
      const result = await publishNacosConfig({ dataId, group: "DEFAULT_GROUP", content: payload, source: "nacos-messages" });
  ...
  useEffect(() => {
    ...
    const sizeClasses: Record<string, string> = { small: "小", "": "正常", large: "大", huge: "特大" };
      ...
      handlers: {
        ...
        eyedropper: async () => {
          ...
          try {
            ...
            if (quillRef.current && result.sRGBHex) quillRef.current.format("color", result.sRGBHex);
          } catch (error: any) {
            if (error?.name !== "AbortError") showToast("颜色提取失败", "error");
          }
        },
      },
    };
    ...
    window.setTimeout(() => {
      ...
      if (htmlBtn) {
        htmlBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:16px;height:16px"><path d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>';
        htmlBtn.setAttribute("title", "编辑HTML源码");
      }
      const eyeBtn = document.querySelector(".ql-eyedropper");
      if (eyeBtn) {
        eyeBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:16px;height:16px"><path d="M20.2 3.8l-2.5 2.5-1.4-1.4 2.5-2.5 1.4 1.4zM4 16l-1 5 5-1 9.5-9.5-4-4L4 16z"/></svg>';
        eyeBtn.setAttribute("title", "屏幕拾色");
      }
      const cleanBtn = document.querySelector(".ql-clean");
      if (cleanBtn) {
        cleanBtn.innerHTML = "🧹 ";
        cleanBtn.setAttribute("title", "选中内容后点此清除颜色和格式，确保继承主题颜色");
      }
      ...
      if (colorPicker) colorPicker.setAttribute("title", "文字颜色 - 建议尽量用默认，如需高亮可选亮色");
      const bgPicker = document.querySelector(".ql-background .ql-picker-label");
      if (bgPicker) bgPicker.setAttribute("title", "背景高亮");
      document.querySelectorAll<HTMLElement>(".ql-size .ql-picker-label").forEach((el) => el.setAttribute("data-label", sizeClasses[el.dataset.value || ""] || "正常"));
      document.querySelectorAll<HTMLElement>(".ql-size .ql-picker-item").forEach((el) => { el.textContent = sizeClasses[el.dataset.value || ""] || "正常"; });
    }, 120);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <PermissionGuard permission="nacos-messages">
      <div className="nacos-section nacos-section-react">
        <div className="nacos-header">
          <div className="nacos-header-title">
            <h2>公告配置</h2>
            <p>可视化编辑 Nacos 配置：xhunt_message / xhunt_message_en</p>
          </div>
        </div>

        <div className="nacos-toolbar">
          <div className="toolbar-left">
            <div className="lang-tabs">
              <button className={`lang-tab ${lang === "zh" ? "active" : ""}`} type="button" onClick={() => switchLang("zh")}>中文</button>
              <button className={`lang-tab ${lang === "en" ? "active" : ""}`} type="button" onClick={() => switchLang("en")}>English</button>
            </div>
            <span className="dataid-label">dataId: <code>{dataId}</code></span>
          </div>
        ...
        <div className="nacos-body">
          <div className="nacos-panel nacos-panel-list">
            ...
            <div className="list-content">
              {sortedItems.length ? sortedItems.map(({ item, index }) => (
                <div key={`${item.created}-${index}`} className={`nacos-msg-item ${index === currentIndex ? "active" : ""}`} onClick={() => selectItem(index)}>
                  <div className="msg-item-title">{item.title || "(无标题)"}</div>
                  <div className="msg-item-meta">{fmtTs(item.created)}</div>
                </div>
              )) : <div className="empty-state">暂无公告，点击「新增公告」创建</div>}
            </div>
          </div>

          <div className="nacos-panel nacos-panel-editor" id="nacos-editor-panel">
            <div className="panel-header">
              <span>编辑器</span>
              <span className="editor-hint">{selectedItem ? `正在编辑: ${selectedItem.created || ""}` : "选择左侧公告开始编辑"}</span>
            </div>
            ...
            <div className="editor-form">
              ...
              <div className="form-field">
                ...
                <div className="editor-color-notice">
                  ...
                  <span><strong>⚠️ 重要提示：</strong>粘贴内容后，请先<strong>全选(Ctrl+A)</strong>然后点击工具栏的 <strong>🧹 清除样式</strong> 按钮，确保文字颜色继承自主题，如需特殊高亮语句清楚颜色后单独再设置颜色。发布后请在白天/黑夜模式下都检查一下显示效果。</span>
                </div>

                <div className={quillReady ? "" : "nacos-hidden"} style={{ display: editorEnabled && quillReady ? "block" : "none" }}>
                  <div ref={quillHostRef} id="nacos-msg-content" />
                </div>

                <div className={quillReady ? "nacos-hidden" : ""}>
                  <div className="rte-toolbar">
                    <button type="button" title="加粗" disabled={!editorEnabled} onClick={() => execFallbackCommand("bold")}> <b>B</b> </button>
                    <button type="button" title="斜体" disabled={!editorEnabled} onClick={() => execFallbackCommand("italic")}> <i>I</i> </button>
                    <button type="button" title="下划线" disabled={!editorEnabled} onClick={() => execFallbackCommand("underline")}> <u>U</u> </button>
                    <button type="button" title="换行" disabled={!editorEnabled} onClick={() => execFallbackCommand("insertHTML", "<br>")}>↵</button>
                    <button type="button" title="插入链接" disabled={!editorEnabled} onClick={() => { const url = window.prompt("输入链接 URL:"); if (url) execFallbackCommand("createLink", url); }}>🔗</button>
                    <button type="button" title="移除链接" disabled={!editorEnabled} onClick={() => execFallbackCommand("unlink")}>✕</button>
                    <span className="toolbar-hint">支持直接粘贴 HTML</span>
                  </div>
                  <div ref={fallbackRef} className="rte-editor" contentEditable={editorEnabled} data-placeholder="请输入公告内容（支持 HTML）" onInput={syncContentFromEditor} />
                </div>
              </div>
        ...
        {toast ? <div className="nacos-toast" style={{ background: toast.type === "error" ? "#991b1b" : toast.type === "success" ? "#065f46" : "#111827" }}>{toast.message}</div> : null}
            ...
            <div className="modal-panel modal-panel-lg">
              ...
              <div className="modal-body">
                <div className="preview-info">
                  ...
                  <span>{diffHint}</span>
                  <div className="legend"><span className="legend-added">新增</span><span className="legend-removed">删除</span></div>
                </div>
