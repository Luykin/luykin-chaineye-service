...
function getDiskRisk(disks?: DeviceStatusResponse["disk"]) {
  ...
  const highest = rows.reduce(
    ...
    },
    {
      value: 0,
      disk: undefined as
        | NonNullable<DeviceStatusResponse["disk"]>[number]
        | undefined,
    },
  );
  ...
  return (
    <Card className={`device-health-card is-${props.level}`}>
      <div className="device-health-card-topline">
        ...
        >
          {healthText(props.level)}
        </Tag>
      </div>
      <div className="device-health-value">
        {props.value.toFixed(props.value % 1 ? 1 : 0)}%
      </div>
      <Progress
        percent={Math.min(100, Math.max(0, props.value))}
        showInfo={false}
        status={progressStatus(props.level)}
        strokeColor={healthColor(props.level)}
        trailColor="#eef2f7"
      />
      <div className="device-health-detail">{props.detail || "-"}</div>
      {props.meta ? (
        <div className="device-health-meta">{props.meta}</div>
      ) : null}
    </Card>
  );
}
...
function StatusPill({ ok, text }: { ok?: boolean; text?: string }) {
  return (
    <Tag color={ok ? "success" : "error"}>{text || (ok ? "正常" : "异常")}</Tag>
  );
}
...
export function DeviceMonitorPage() {
  ...
  const query = useQuery({
    ...
    enabled: hasQueried,
    refetchInterval: hasQueried ? 60_000 : false,
  });
  ...
  const sseTotalConnections = Number(
    (data?.sse?.connections as { active?: number } | undefined)?.active || 0,
  );

  const pm2Columns: ColumnsType<
    NonNullable<DeviceStatusResponse["pm2"]>[number]
  > = [
    {
      title: "进程",
      dataIndex: "name",
    ...
    },
    {
      ...
      width: 110,
      render: (value?: string) => (
        <Tag color={value === "online" ? "success" : "error"}>
          {value || "-"}
        </Tag>
      ),
    },
  ...
  > = [
    ...
    },
    {
      ...
      key: "usage",
      width: 160,
      render: (_, record) => `${record.used || "-"} / ${record.size || "-"}`,
    },
  ...
  const sseColumns: ColumnsType<SseProcessRow> = [
    {
      ...
      width: 180,
      render: (value?: string) => (
        <Typography.Text code>{value || "current"}</Typography.Text>
      ),
    },
    {
      title: "连接数",
    ...
    },
    {
      ...
      render: (value?: boolean) => (
        <StatusPill ok={value} text={value ? "已初始化" : "未初始化"} />
      ),
    },
    {
      title: "Feed 轮询",
      dataIndex: "feedIntervalActive",
      key: "feedIntervalActive",
      width: 110,
      render: (value?: boolean) => (
        <StatusPill ok={value} text={value ? "运行中" : "未启动"} />
      ),
    },
    {
      title: "Top Tweet",
      dataIndex: "topTweetIntervalActive",
      key: "topTweetIntervalActive",
      width: 110,
      render: (value?: boolean) => (
        <StatusPill ok={value} text={value ? "运行中" : "未启动"} />
      ),
    ...
    <PermissionGuard permission="device-status:read">
      ...
      >
        ...
        {!data && !query.isError ? (
          <Empty
            description={
              hasQueried ? "暂无设备状态数据" : "点击查询后加载设备状态"
            }
          />
        ) : null}
        {data ? (
          ...
          >
              ...
              <div className="device-overview-main">
                <div>
                  ...
                  <div className="device-overview-desc">
                    主机 {data.system?.hostname || "-"} ·{" "}
                    {data.system?.platform || "-"} · 运行{" "}
                    {data.system?.uptime || "-"}
                  </div>
                </div>
                <div className="device-overview-time">
                  更新时间：{data.timestamp}
                </div>
              </div>
            </Card>
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={8}>
                <MetricCard
                  title="磁盘占用"
                  value={diskRisk.value}
                  level={diskRisk.level}
                  detail={`${diskRisk.disk?.mounted || "-"} · ${diskRisk.disk?.used || "-"} / ${diskRisk.disk?.size || "-"}`}
                  meta="超过 90% 需要立即处理"
                />
              </Col>
              <Col xs={24} lg={8}>
              ...
              <Col xs={24} lg={8}>
                <MetricCard
                  ...
                  value={cpuPercent}
                  level={cpuLevel}
                  detail={`${data.cpu?.cores || "-"} 核 · ${data.cpu?.loadAverage || "-"}`}
                  meta={data.cpu?.model || "-"}
                />
              </Col>
            </Row>
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} xl={16}>
                <Card
                  title={
                    <Space>
                      <span>PM2 运行</span>
                      <Badge
                        status={pm2Bad ? "error" : "success"}
                        text={`${pm2Online}/${pm2Rows.length} online`}
                      />
                    </Space>
                  }
                ...
                >
                  <Table
                    rowKey={(record) =>
                      [record.name, record.status, record.uptime]
                        .filter(Boolean)
                        .join("-")
                    }
                    columns={pm2Columns}
                    dataSource={pm2Rows}
                    scroll={{ y: TABLE_MAX_HEIGHT, x: 780 }}
                    pagination={false}
                    size="small"
                    locale={{
                      emptyText: <Empty description="暂无 PM2 数据" />,
                    }}
                  />
                </Card>
              </Col>
              <Col xs={24} xl={8}>
                <Card
                  title="数据库状态"
                  className="device-panel device-db-panel"
                >
                  <div className="device-db-row">
                    <span>Redis</span>
                    <StatusPill
                      ok={data.redis?.connected}
                      text={data.redis?.connected ? "已连接" : "未连接"}
                    />
                  </div>
                  <div className="device-db-meta">
                    内存 {data.redis?.memory || "-"} /{" "}
                    {data.redis?.maxMemory || "未设置"}
                  </div>
                  <div className="device-db-meta">
                    Key 数量 {data.redis?.keys ?? "-"} · 运行{" "}
                    {data.redis?.uptime || "-"}
                  </div>
                  ...
                  <div className="device-db-row">
                    ...
                    <StatusPill
                      ok={data.postgresql?.connected}
                      text={data.postgresql?.connected ? "已连接" : "未连接"}
                    />
                  </div>
                  <div className="device-db-meta">
                    版本 {data.postgresql?.version || "-"}
                  </div>
                  <div className="device-db-meta">
                    大小 {data.postgresql?.size || "-"} · 活跃连接{" "}
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} xl={12}>
                <Card title="磁盘详情" className="device-panel">
                  <Table
                    rowKey={(record) =>
                      `${record.filesystem}-${record.mounted}`
                    }
                    columns={diskColumns}
              ...
              <Col xs={24} xl={12}>
                ...
                >
                  <Table
                    rowKey={(record, index) =>
                      `${record.processId || "current"}-${index}`
                    }
                    columns={sseColumns}
                    dataSource={sseRows}
                    pagination={false}
                    ...
                    scroll={{ y: 300, x: 620 }}
                    locale={{
                      emptyText: <Empty description="暂无 SSE 进程连接数据" />,
                    }}
                  />
                </Card>
              </Col>
