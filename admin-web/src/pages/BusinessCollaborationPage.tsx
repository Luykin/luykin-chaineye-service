...
const assigneeLabel = (user: { displayName?: string | null; accountName?: string | null; primaryGoogleEmail?: string | null; twitterHandle?: string | null; id: string }) => `${user.displayName || user.accountName || user.primaryGoogleEmail || "未命名用户"}${user.twitterHandle ? ` · @${user.twitterHandle}` : ""}`;
...
export function BusinessCollaborationPage() {
  ...
  const assigneeQueryResult = useQuery({ queryKey: ["business-collaboration-assignees", assigneeQuery], queryFn: () => fetchCollaborationAssignees(assigneeQuery), enabled: editorOpen || !!accessActivity });
  ...
  const openEdit = (item?: CollaborationActivity) => {
    setEditing(item || null); setEditorOpen(true); setAssigneeQuery("");
  ...
  const assigneeOptions = (assigneeQueryResult.data?.data || []).map((user) => ({ value: user.id, label: assigneeLabel(user) }));
  const columns = [
  ...
  return <PermissionGuard permission="business_collaboration_manage"><PageSection title="定向合作活动" description="选择项目 X 账号后自动带出资料，并为项目方人员分配可见和代发邀约权限。"><>{contextHolder}<Card extra={<Space><Button icon={<ReloadOutlined />} onClick={refresh}>刷新</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => openEdit()}>新建活动</Button></Space>}><Table rowKey="id" loading={query.isLoading} columns={columns} dataSource={query.data?.data || []} pagination={false} /></Card><Modal open={editorOpen} title={editing ? "编辑定向合作活动" : "新建定向合作活动"} width={920} onCancel={() => { setEditorOpen(false); setEditing(null); setProjectAccount(null); }} onOk={() => form.submit()} confirmLoading={save.isPending} destroyOnClose><Form form={form} layout="vertical" onFinish={submit}><Row gutter={20}>
    ...
    <Col xs={24} md={10}><Form.Item name="startAt" label="开始时间" rules={[{ required: true }]}><Input type="datetime-local" /></Form.Item></Col><Col xs={24} md={10}><Form.Item name="endAt" label="结束时间" rules={[{ required: true }]}><Input type="datetime-local" /></Form.Item></Col><Col xs={12} md={2}><Form.Item name="reviewerMode" label="审核方"><Select options={[{ value: "project", label: "项目方" }, { value: "echohunt", label: "EchoHunt" }]} /></Form.Item></Col><Col xs={12} md={2}><Form.Item name="status" label="状态"><Select options={statuses.map((value) => ({ value, label: value }))} /></Form.Item></Col>
    {!editing && <Col span={24}><Form.Item name="authCenterUserIds" label="项目方人员" extra="这些人创建后即可在 EchoHunt 中查看本活动，并代表项目方给 KOL 发送邀约。"><Select mode="multiple" showSearch filterOption={false} options={assigneeOptions} placeholder="搜索姓名、邮箱或 X Handle 后选择（可多选）" onSearch={setAssigneeQuery} notFoundContent={assigneeQueryResult.isFetching ? "搜索中…" : "未找到用户"} /></Form.Item></Col>}
