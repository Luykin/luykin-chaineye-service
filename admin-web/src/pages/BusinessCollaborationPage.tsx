...
import { AutoComplete, Button, Card, Col, Descriptions, Form, Input, InputNumber, Modal, Popconfirm, Progress, Row, Select, Space, Table, Tag, Tooltip, Typography, message } from "antd";
...
export function BusinessCollaborationPage() {
  ...
  const columns = [
    { title: "活动 / 项目 X", key: "name", width: 280, render: (_: unknown, row: CollaborationActivity) => <><Typography.Text strong>{row.name}</Typography.Text><br /><Typography.Text type="secondary">{row.projectDisplayName || "-"} · @{row.projectTwitterHandle || "-"}</Typography.Text></> },
