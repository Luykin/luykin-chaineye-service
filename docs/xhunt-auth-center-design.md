...
|------|------|------|------|
| `id` | UUID | 是 | 主键 |
| `accountName` | STRING | 否 | 用户自己设置的账户名或邮箱，账户密码登录使用，唯一 |
| `accountNameLower` | STRING | 否 | 小写账户名或邮箱，用于唯一索引和登录查询 |
...
- 普通账户名长度 3 - 32。
- 普通账户名允许字母、数字、下划线、短横线。
- 普通账户名不允许纯数字。
- 也允许使用邮箱作为账户名，例如 `name@example.com`，最长 255 个字符。
