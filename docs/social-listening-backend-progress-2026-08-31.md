...
- 召回口径：关键词、别名、token、官方 handle、项目名合并成一组召回词；不引入“强词/弱词”复杂配置。命中 `metadata.recallExcludeKeywords` 的帖子直接不入库。
- 词云口径：不再直接展示召回关键词，而是聚合 AI `hot_tags` 写入的讨论热词；排除项目名、官方 handle、token、aliases、召回 keywords，并支持看板级 `metadata.wordCloudExcludeKeywords` 继续追加排除词。
...
| 词云就是召回关键词列表 | 当前词云应表达“大家在讨论什么”，聚合 AI 热词并过滤召回/品牌词；运营可在看板里配置词云排除词。 |
| 召回需要强词/弱词多层规则 | 当前保持简单：只有召回词和召回排除词；召回排除词命中后直接丢弃。 |
