...
> 当前实现说明（2026-06-26）：这套签名已改为轻量固定模式。认证中心后端强制校验，不再通过环境变量切换 off/report/enforce；前后端使用代码内固定 public salt：`xhunt-web-sign-w1-fixed-lite-20260626`。该值会随前端包暴露，只用于轻量防裸调、防重放和请求体校验，不作为真正 secret。
