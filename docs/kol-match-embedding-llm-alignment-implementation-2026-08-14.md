...
7. 第二次 LLM 候选深评改为分批并发执行（默认 10 个候选/批）：单批漏评或输出校验失败时只用 proxy 补齐缺失候选，保留已成功的 LLM 深评结果；全部分批失败时才整体进入“基础匹配模式”。结果页新增“部分完成”状态，避免串行分批导致 SSE 超时断流。

## 4. 风险与边界

- 已接入项目 X 画像证据：前端把账号 lookup 结果作为 `xProfile` 传给 `/strategy`，但后端会优先自己调用 `fetch/twitter/user` lookup，并整理 `x:identity`、`x:bio`、`x:narrative`、`x:mention_summary`、`x:post:*`（上游有返回时）给第一次 LLM，同时返回 `profileContext` 供确认策略页展示；内部 X lookup 异常只 retry 1 次，不再 PG fallback；若缺少 narrative / mention_summary / Bio / 近期内容，则仍以 brief 和硬筛为准。
...
2026-08-15 后续线上复测出现 `Evaluator returned 16/40 valid assessments`：说明第二次 LLM 已调用成功，但一次性评测 40 个候选时只返回/通过校验 16 个 assessment，旧逻辑因此整批 fallback。现已改为分批并发深评并支持部分成功补齐：例如 16 个候选通过 LLM 校验时，保留这 16 个深评结果，剩余候选用 proxy 补齐，`meta.evaluation.partial=true`，前端显示“深度评测：部分完成”，不再直接显示“基础匹配模式”。并发执行可把最坏等待从多个批次串行累加压到单批超时窗口内，降低 SSE network error 风险。
